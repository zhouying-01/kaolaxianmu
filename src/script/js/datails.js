;!function(){
        ...
        const $spic=$('#showImgBox').find('img');
        const $sf=$('.sf');
        const $bf=$('#showDetails');
        const $bpic=$('#showImgBig');
        const $title=$('.product-title').find('span');
        const $nowprice=$('.currentPrice').find('span');
        const $price=$('.marketPrice').find('span');
        const $zk=$('#zk')
        ...
        $.ajax({
            ...
            dataType:'json'
              ...
              let $rebate=(Number((data.price))/Number(data.nowprice)*10).toFixed(1)+'折';
              $zk.html($rebate);
    ...
    //   购物车
     ...
     function cookievalue(){
       ...
       $('#addCart').on('click',function(){//点击加入购物车按钮，将商品数量加入cookie
               alert('成功加入购物车')
            cookievalue();//查看是否有存在的cookie值
            if($.inArray($sid,arrsid)!=-1){//存在值就想加
